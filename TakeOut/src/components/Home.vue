<template>
    <div >
      <!-- 头部导航栏  NavBar 导航栏 -->
        <div class="nav-bar">
          <van-nav-bar title="标题"  fixed style="background: #00A67C; ">
            <template #left >
              <van-icon name="search" size="18" />
            </template>
            <template #right>
                <p> <a :href="'#/login'">登陆 </a> | <a :href="'#/login'">注册</a> </p>
            </template>
          </van-nav-bar>
        </div>
        <!-- 主要商品分类导航  Grid 宫格 -->
        <div class="sudoku">
            <van-grid :column-num="4" :border="false">
              <van-grid-item v-for="item in foodCategory" :key="item.id" :icon="'http://47.95.13.193:80/takeOutSystem-1.0-SNAPSHOT/'+item.photo" :text="item.name" />
            </van-grid>
        </div>
<!--      附近商家-->
